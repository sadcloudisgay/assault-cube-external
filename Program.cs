...
class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            IntPtr enemyPlayerAddress = IntPtr.Zero; // Declare outside the loop
            ...
            for (int i = 0; i < playerCount; i++)
            {
                ...
                // Set the enemy player address for later use
                enemyPlayerAddress = entityPointer; // Assign the current enemy's address
            ...
            if ((GetAsyncKeyState(VK_RBUTTON) & 0x8000) != 0 && closestAngleDifference != float.MaxValue && renderer.Aimbot)
            {
                ...
                // Normalize targetYaw to the range [-180, 180]
                targetYaw = ClampAngle(targetYaw);

                // Calculate yaw and pitch differences
                float yawDifference = ClampAngle(targetYaw - localYaw);
                float pitchDifference = ClampAngle(targetPitch - localPitch);
            ...
            if ((GetAsyncKeyState(VK_MOUSE1) & 0x8000) != 0 && closestAngleDifference != float.MaxValue && renderer.enemyteleport)
            {
                // Ensure we have a valid closest enemy head position
                if (closestHeadPosition != Vector3.Zero && localHeadPosition != Vector3.Zero)
                {
                    // Calculate the forward direction from the local player’s yaw and pitch
                    float yawRad = localYaw * (float)(Math.PI / 180.0); // Convert yaw to radians
                    float pitchRad = localPitch * (float)(Math.PI / 180.0); // Convert pitch to radians
                    ...
                    // Set a distance for how far the enemy should be teleported in front of the player
                    float teleportDistance = 0f; // Adjust this value for how far in front you want to teleport the enemy

                    // Calculate the new enemy position (in front of the local player)
                    Vector3 enemyNewPosition = localHeadPosition + (forwardDirection * teleportDistance);

                    // Write the new position for the enemy (teleport the enemy in front of the local player)
                    window.WriteVec(enemyPlayerAddress + 0x4, enemyNewPosition); // Assuming enemy head position offset is 0x4

                    // Introduce a small delay to prevent shooting walls
                    long targetDelay = 0; // Delay in microseconds
                    Stopwatch stopwatch = new Stopwatch();
                    stopwatch.Start();
                    while (stopwatch.ElapsedTicks * 1000 / Stopwatch.Frequency < targetDelay)
                    {
